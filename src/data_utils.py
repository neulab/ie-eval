"""A modified version of the original data preprocessor at harvardnlp/data2text

    Added:
        - German keywords.
        - annoying_number_word fix.
...
from text2num import NumberException, text2num
...
def get_ents(dat):
    """Construct a set of possible name combinationations."""
    players = set()
    teams = set()
    cities = set()
    for thing in dat:
...
def deterministic_resolve(
    ...
    for j in range(len(curr_ents) - 1, -1, -1):
        if pron in Keywords.singular and curr_ents[j][2] in players:
            return None
        elif pron in Keywords.plural and curr_ents[j][2] in teams:
            return None
        elif pron in Keywords.plural and curr_ents[j][2] in cities:
            return None
    ...
    if len(prev_ents) > 0:
        for i in range(len(prev_ents) - 1, len(prev_ents) - 1 - max_back, -1):
            for j in range(len(prev_ents[i]) - 1, -1, -1):
                if pron in Keywords.singular and prev_ents[i][j][2] in players:
                    return prev_ents[i][j]
                elif pron in Keywords.plural and prev_ents[i][j][2] in teams:
                    return prev_ents[i][j]
                elif pron in Keywords.plural and prev_ents[i][j][2] in cities:
                    return prev_ents[i][j]
    return None
    ...
    while i < len(sent):
        ...
        if sent[i] in prons:
            if resolve_prons:
                ...
                if referent is None:
                    sent_ents.append(
                        EntitySpan(i, i + 1, sent[i], True)
                    )  # is a pronoun
                else:
                    # pretend it's not a pron and put in matching string
                    sent_ents.append(EntitySpan(i, i + 1, referent[2], False))
            else:
                sent_ents.append(EntitySpan(i, i + 1, sent[i], True))  # is a pronoun
            i += 1
        ...
        # findest longest spans; only works if we put in words...
            ...
            while i + j <= len(sent) and " ".join(sent[i : i + j]) in all_ents:
                j += 1
            sent_ents.append(
                EntitySpan(i, i + j - 1, " ".join(sent[i : i + j - 1]), False)
            )
            i += j - 1
        else:
            i += 1
...
def annoying_number_word(sent, i):
    """Original seems wrong. Fixed the "not"s"""
    return (
        " ".join(sent[i : i + 3]) in Keywords.ignores
        and " ".join(sent[i : i + 2]) in Keywords.ignores
    )
...
def extract_numbers(sent) -> List[NumberSpan]:
    ...
    while i < len(sent):
        ...
        if toke.isnumeric():
            ...
            while (
                i + j < len(sent)
                and sent[i + j] in Keywords.number
...
def get_player_idx(bs, entname):
    ...
    if len(keys) == 0:
        ...
        if (
            len(keys) > 1
        ):  # if we matched on first name and there are a bunch just forget about it
            return None
    assert len(keys) <= 1, entname + " : " + str(bs["PLAYER_NAME"].values())
    return keys[0] if len(keys) > 0 else None


def get_rels(entry, ents, nums, players, teams, cities) -> List[Relation]:
    """
    ...
    for i, ent in enumerate(ents):
        ...
        if entname in players and entname not in cities and entname not in teams:
                ...
                if (
                    ...
                    for colname, col in bs.items():
                        if col[pidx] == strnum:  # allow multiple for now
                            rels.append(
                                Relation(ent, numtup, "PLAYER-" + colname, pidx)
                            )
                            found = True
                if not found:
                    rels.append(Relation(ent, numtup, "NONE", None))
            ...
            if (
                ...
                entpieces[0] in entry["vis_city"] or entpieces[-1] in entry["vis_name"]
            ):
                ...
                if linescore is not None:
                    for colname, val in linescore.items():
                        if val == strnum:
                            ...
                            rels.append(Relation(ent, numtup, colname, is_home))
                            found = True
                if not found:
                    rels.append(
                        Relation(ent, numtup, "NONE", None)
                    )  # should i specialize the NONE labels too?
    return rels


def append_candidate_rels(
    entry, summ, all_ents, prons, players, teams, cities, candrels
):
    """
    ...
    for j, sent in enumerate(sents):
        ...
        tokes: List[str] = sent.split()
        ents: List[EntitySpan] = extract_entities(tokes, all_ents, prons)
        nums: List[NumberSpan] = extract_numbers(tokes)
        rels: List[Relation] = get_rels(entry, ents, nums, players, teams, cities)
        if len(rels) > 0:
...
def get_datasets(path="../boxscore-data/rotowire"):
    ...
    for dataset in datasets:
        nugz = []
        for i, entry in tqdm(
            enumerate(dataset), ncols=80, ascii=True, desc="Exting candidates"
        ):
            # this has to be the normal summary field, when making data for training full english model.
            summ = " ".join(entry[f"summary_{LANG}"])
            append_candidate_rels(
                entry, summ, all_ents, Keywords.prons, players, teams, cities, nugz
            )
...
def append_to_data(
    tup, sents, lens, entdists, numdists, labels, vocab, labeldict, max_len
):
    """
    tup is (sent, [rels]);
    ...
    for rel in tup[1]:
        ...
        lens.append(sentlen)
        ent_dists = [
            j - ent.start if j < ent.start else j - ent.end + 1 if j >= ent.end else 0
...
def save_full_sent_data(
    ...
    for k in list(word_counter.keys()):
        if word_counter[k] < min_freq:
            del word_counter[k]  # will replace w/ unk
    vocab = dict(((wrd, i + 2) for i, wrd in enumerate(word_counter.keys())))
    vocab["UNK"] = 0
    vocab["PAD"] = 1
    ...
    print
    "max tr sentence length:", max_trlen

    # do training data
    for tup in tqdm(datasets[0], ncols=80, ascii=True):
        if multilabel_train:
            append_multilabeled_data(
                tup,
                trsents,
                trlens,
                trentdists,
    ...
    if nonedenom > 0:
        ...
        print
        "originally", len(trlabels), "training examples"
        print
        "keeping", num_to_keep, "NONE-labeled examples"
        ...
        trentdists = [
            thing for i, thing in enumerate(trentdists) if i not in ignore_idxs
        ]
        trnumdists = [
            thing for i, thing in enumerate(trnumdists) if i not in ignore_idxs
        ]
    ...
    for tup in tqdm(datasets[1], ncols=80, ascii=True):
    ...
    for tup in tqdm(datasets[2], ncols=80, ascii=True):
        # append_to_data(tup, valsents, vallens, valentdists, valnumdists, vallabels, vocab, labeldict, max_len)
        append_multilabeled_data(
            tup,
            testsents,
    ...
    data = {}
    data["trsents"] = trsents
    data["trlens"] = trlens
    data["trentdists"] = trentdists
    data["trnumdists"] = trnumdists
    data["trlabels"] = trlabels
    data["valsents"] = valsents
    data["vallens"] = vallens
    data["valentdists"] = valentdists
    data["valnumdists"] = valnumdists
    data["vallabels"] = vallabels

    data["testsents"] = testsents
    data["testlens"] = testlens
    data["testentdists"] = testentdists
    data["testnumdists"] = testnumdists
    ...
    with open(outfile.split(".")[0] + ".dict", "w+") as f:
        for i in range(len(revvocab)):
            f.write("%s %d \n" % (revvocab[i], i))

    with open(outfile.split(".")[0] + ".labels", "w+") as f:
        for i in range(len(revlabels)):
            f.write("%s %d \n" % (revlabels[i], i))


def prep_generated_data(
    genfile, dict_pfx, outfile, path="../boxscore-data/rotowire", test=False
):
    # recreate vocab and labeldict
    vocab = {}
    with open(dict_pfx + ".dict", "r") as f:
        for line in f:
            pieces = line.strip().split()
            vocab[pieces[0]] = int(pieces[1])

    labeldict = {}
    with open(dict_pfx + ".labels", "r") as f:
        for line in f:
            pieces = line.strip().split()
    ...
    key2idx = {v["id"]: i for i, v in enumerate(valdata)}
    idx2key = {v: k for k, v in key2idx.items()}
    key2genentry = {v["id"]: v for v in gens}

    nugz = []  # to hold (sentence_tokens, [rels]) tuples
    sent_reset_indices = {0}  # sentence indices where a box/story is reset
    # for i, gen_entry in enumerate(gens):
    # Enforce the same order as the test set.
    for i, k in idx2key.items():
        entry = valdata[i]
        gen_entry = key2genentry[k]
        summ = " ".join(gen_entry[f"summary"])
        append_candidate_rels(
            entry, summ, all_ents, Keywords.prons, players, teams, cities, nugz
        )
    ...
    for t, tup in enumerate(nugz):
        ...
        append_multilabeled_data(
            tup, psents, plens, pentdists, pnumdists, plabels, vocab, labeldict, max_len
        )
    ...
    data = {}
    data["valsents"] = psents
    data["vallens"] = plens
    data["valentdists"] = pentdists
    data["valnumdists"] = pnumdists
    data["vallabels"] = plabels
    data["boxrestartidxs"] = np.array(
...
def fix_target_idx(summ, assumed_idx, word, neighborhood=5):
    ...
    for i in range(1, neighborhood + 1):
        if assumed_idx + i < len(summ) and summ[assumed_idx + i] == word:
            ...
            assumed_idx - i >= 0
            and assumed_idx - i < len(summ)
            and summ[assumed_idx - i] == word
        ):
            return assumed_idx - i
    return None
    ...
    for i, entry in enumerate(trdata):
        ...
        for j, sent in enumerate(sents):
            ...
            ents = extract_entities(
                tokes, all_ents, prons, prev_ents, resolve_prons, players, teams, cities
            )
            if resolve_prons:
                    ...
                    if isinstance(idthing, bool):  # city or team
                        ...
                        for k, word in enumerate(tokes[ent_start:ent_end]):
                            ...
                            if src_idx is not None:
                                ...
                                if (
                                    targ_idx >= len(entry[f"summary_{LANG}"])
                                    or entry[f"summary_{LANG}"][targ_idx] != word
                                ):
                                    targ_idx = fix_target_idx(
                                        entry[f"summary_{LANG}"], targ_idx, word
                                    )
                                if targ_idx is None:
                                    skipped += 1
                                else:
                                    assert (
                                        ...
                                        and entry[f"summary_{LANG}"][targ_idx] == word
                                    )
                                    links.append(
                                        (src_idx, targ_idx)
                                    )  # src_idx, target_idx
                            ...
                            if idthing:  # home, so look in the home row
                                if entry["home_line"][label] == word:
                                    ...
                                    src_idx = (
                                        ...
                                        + col_idx
                                        - 1
                                    )  # -1 b/c we trim first col
                            else:
                                if entry["vis_line"][label] == word:
                                    col_idx = ls_keys.index(label)
                                    src_idx = (
                                        (2 * NUM_PLAYERS + 1) * (len(bs_keys) - 1)
                                        + len(bs_keys)
                                        - len(ls_keys)
                                        + col_idx
                                        - 1
                        ...
                        if player_row is not None:
                            ...
                            for k, word in enumerate(tokes[ent_start:ent_end]):
                                ...
                                if src_idx is not None:
                                    targ_idx = words_so_far + ent_start + k
                                    if entry[f"summary_{LANG}"][targ_idx] != word:
                                        targ_idx = fix_target_idx(
                                            entry[f"summary_{LANG}"], targ_idx, word
                                        )
                                    if targ_idx is None:
                                        ...
                                        assert (
                                            rulsrcs[i][src_idx] == word
                                            and entry[f"summary_{LANG}"][targ_idx]
                                            == word
                                        )
                                        links.append(
                                            (src_idx, targ_idx)
                            ...
                            for k, word in enumerate(tokes[num_start:num_end]):
                                ...
                                if (
                                    word
                                    == entry["box_score"][label.split("-")[1]][idthing]
                                ):
                                    src_idx = (
...
def save_coref_task_data(outfile, inp_file="full_newnba_prepdata2.json"):
    ...
    max_vallen = max((len(tup[0]) for tup in datasets[1]))
    print
    "max sentence lengths:", max_trlen, max_vallen
    ...
    print
    len(trwindows), "training examples"
    print
    len(valwindows), "validation examples"
    print
    Counter(trlabels)
    print
    Counter(vallabels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-path", type=str, default="", help="path to input")
    parser.add_argument(
        "--output-fi", type=str, default="", help="desired path to output file"
    )
    parser.add_argument(
        "--gen-fi",
        type=str,
    ...
    parser.add_argument(
        "--lang", type=str, choices=["en", "de"], help="language", default="de"
    )
