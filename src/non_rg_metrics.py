"""A modified script from ratishsp/data2text-1

    Usage: python non_rg_metrics.py gold system

    Modification:
        - Format coding style to black
        - Port to python3
"""
import sys
...
def same_ent(e1, e2):
    if e1 in cities or e1 in teams or e2 in cities or e2 in teams:
        return e1 == e2 or any(
            (e1 in fullname and e2 in fullname for fullname in full_names)
        )
    else:
...
def trip_match(t1, t2):
    return (
        int_value(t1[1]) == int_value(t2[1])
        and t1[2] == t2[2]
        and same_ent(t1[0], t2[0])
    )
...
def calc_precrec(goldfi, predfi):
    ...
    for i, triplist in enumerate(pred_triples):
        tp = sum(
            (
                ...
                if any(
                    trip_match(triplist[j], gold_triples[i][k])
    ...
    print(f" CS Prec: {avg_prec*100:2.2f}\n"
          f" CS Rec:  {avg_rec*100:2.2f}")
